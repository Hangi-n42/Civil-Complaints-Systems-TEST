"""Week5 LLM 모델 동일조건 벤치마크 스크립트.

Week4 AX4 전용 스크립트에서 검증한 answer 복구 로직을
다른 모델에도 동일하게 적용하는 범용 벤치마크 버전이다.

Usage:
  python scripts/Be3_run_week5_model_benchmark.py \
            ...
            for rep in range(repetitions):
                ...
                try:
                    ...
                    if not integrity_passed_initial:
                        ...
                        strict_citations = strict_citations_raw if isinstance(strict_citations_raw, list) else []
                        strict_answer = _derive_non_empty_answer(parsed_compact, raw_response, case["context"])
                        strict_cite_rate = _citation_match_rate(strict_citations, case["context"])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Week5 LLM 모델 벤치마크")
    parser.add_argument(
        "--config",
        type=str,
